...
test_reshape :: proc(t: ^testing.T) {
	// Test basic 2D to 1D reshape
	{
		...
		testing.expect(t, slice.equal(reshaped.shape, expected_shape), "2D to 1D reshape shape incorrect")
	...
	// Test 1D to 3D reshape
	{
		...
		// Check shape
		expected_shape := []uint{2, 2, 2}
		testing.expect(t, slice.equal(reshaped.shape, expected_shape), "1D to 3D reshape shape incorrect")
	...
	// Test 3D to 2D reshape
	{
		...
		testing.expect(t, slice.equal(reshaped.shape, expected_shape), "3D to 2D reshape shape incorrect")
	...
	// Test reshape with non-contiguous tensor (transposed)
	{
		...
		testing.expect(t, slice.equal(reshaped.shape, expected_shape), "Transposed reshape shape incorrect")

		// After transpose [[1,2],[3,4]] becomes [[1,3],[2,4]], so flattened should be [1,3,2,4]
		expected_data := []f32{1, 3, 2, 4}
		for i in 0 ..< 4 {
			testing.expect(t, reshaped.data[i] == expected_data[i], 
				fmt.tprintf("Transposed reshape data mismatch at %d: got %f, expected %f", 
					i, reshaped.data[i], expected_data[i]))
		}
	}
	...
	// Test same shape reshape (no-op)
	{
		...
		testing.expect(t, slice.equal(reshaped.shape, expected_shape), "Same shape reshape incorrect")
	...
	// Test 4D to 2D reshape (common in ML)
	{
		...
		testing.expect(t, slice.equal(reshaped.shape, expected_shape), "4D to 2D reshape shape incorrect")

		// Check data preservation
		for i in 0 ..< 24 {
			testing.expect(t, reshaped.data[i] == f32(i + 1), fmt.tprintf("Data mismatch at %d", i))
		}
	}
}
...
test_sequential_non_contiguous_operations :: proc(t: ^testing.T) {
	...
	// Test 1: permute -> reshape -> matmul
	{
		...
		testing.expect(t, !permuted.contiguous, "Permuted tensor should be non-contiguous")
		...
		testing.expect(t, reshaped.contiguous, "Reshaped tensor should be contiguous")
	...
	// Test 2: transpose -> chunk -> operations on chunks
	{
		...
		testing.expect(t, slice.equal(transposed.shape, []uint{2, 4, 2, 3}), "Transpose shape incorrect")
		...
		testing.expect(t, len(chunks) == 2, "Should have 2 chunks")
		for chunk_tensor in chunks {
			testing.expect(t, !chunk_tensor.contiguous, "Chunk should be non-contiguous")
			testing.expect(t, slice.equal(chunk_tensor.shape, []uint{2, 2, 2, 3}), "Chunk shape incorrect")
		}
		
		// Reshape each chunk and verify data consistency
		for chunk_tensor in chunks {
			reshaped_chunk := reshape(chunk_tensor, []uint{24}, context.temp_allocator)
			testing.expect(t, reshaped_chunk.contiguous, "Reshaped chunk should be contiguous")
			testing.expect(t, len(reshaped_chunk.data) == 24, "Reshaped chunk data length incorrect")
		}
	}
	...
	// Test 3: Multiple transposes in sequence
	{
		...
		testing.expect(t, !first_transpose.contiguous, "First transpose should be non-contiguous")
		testing.expect(t, slice.equal(first_transpose.shape, []uint{3, 2, 4}), "First transpose shape incorrect")
		
		// Second transpose: swap dims 1 and 2 -> [3, 4, 2]
		second_transpose := transpose(first_transpose, 1, 2, context.temp_allocator)
		testing.expect(t, !second_transpose.contiguous, "Second transpose should be non-contiguous")
		testing.expect(t, slice.equal(second_transpose.shape, []uint{3, 4, 2}), "Second transpose shape incorrect")
		
		// Verify data correctness by accessing specific elements
		// Get strided data to verify correctness
		strided_data, allocated := get_strided_data(second_transpose, allocator = context.temp_allocator)
	...
	// Test 4: permute -> transpose -> reshape chain
	{
		...
		testing.expect(t, !permuted.contiguous, "Permuted should be non-contiguous")
		testing.expect(t, slice.equal(permuted.shape, []uint{3, 2, 2, 2}), "Permute shape incorrect")
		
		// Transpose dims 2,3 -> [3, 2, 2, 2] (same shape but different strides)
		transposed := transpose(permuted, 2, 3, context.temp_allocator)
		testing.expect(t, !transposed.contiguous, "Transposed should be non-contiguous")
		
		// Reshape to 2D [3, 8]
		reshaped := reshape(transposed, []uint{3, 8}, context.temp_allocator)
		testing.expect(t, reshaped.contiguous, "Final reshape should be contiguous")
		testing.expect(t, slice.equal(reshaped.shape, []uint{3, 8}), "Final reshape shape incorrect")
	...
	// Test 5: Operations on non-contiguous tensor with arithmetic operations
	{
		...
		testing.expect(t, !a_transposed.contiguous, "A transposed should be non-contiguous")
		testing.expect(t, !b_transposed.contiguous, "B transposed should be non-contiguous")
		...
		testing.expect(t, slice.equal(sum_result.shape, []uint{3, 2}), "Sum result shape incorrect")
...
test_contiguousness_edge_cases :: proc(t: ^testing.T) {
	...
	// Test 1: transpose same dimension (should remain contiguous)
	{
		...
		testing.expect(t, same_transpose.contiguous, "Same dimension transpose should remain contiguous")
		
		// Data should be identical
		for i in 0 ..< len(data) {
			testing.expect(t, same_transpose.data[i] == data[i], "Same transpose data should be identical")
		}
	}
	...
	// Test 2: permute with identity permutation (should remain contiguous)
	{
		...
		testing.expect(t, slice.equal(identity_permute.shape, tensor.shape), "Identity permute shape should be same")
		
		// Verify data correctness through strided access
		strided_data, allocated := get_strided_data(identity_permute, allocator = context.temp_allocator)
		testing.expect(t, allocated, "Should have allocated strided data")
		for i in 0 ..< len(data) {
			testing.expect(t, strided_data[i] == data[i], "Identity permute data should be identical")
		}
	}
	...
	// Test 3: chunk with single group (should be same as original but non-contiguous)
	{
		...
		testing.expect(t, !chunks[0].contiguous, "Single group chunk should be non-contiguous")
		testing.expect(t, slice.equal(chunks[0].shape, tensor.shape), "Single group chunk shape should match original")
		
		// Data should be accessible correctly
		strided_data, allocated := get_strided_data(chunks[0], allocator = context.temp_allocator)
		testing.expect(t, allocated, "Should have allocated strided data")
		for i in 0 ..< len(data) {
			testing.expect(t, strided_data[i] == data[i], "Single group chunk data should match original")
		}
	}
	...
	// Test 4: operations that should result in contiguous output from non-contiguous input
	{
		...
		testing.expect(t, reshaped.contiguous, "Reshape result should be contiguous")
		
		// 2. clone
		cloned := clone(transposed, context.temp_allocator)
		testing.expect(t, !cloned.contiguous, "Clone should preserve original contiguousness flag")
		
		// 3. element-wise operations
		same_shape := new_with_init([]f32{1, 1, 1, 1, 1, 1, 1, 1}, []uint{4, 2}, context.temp_allocator)
		
		add_result := add(transposed, same_shape, context.temp_allocator)
		testing.expect(t, add_result.contiguous, "Add result should be contiguous")
		testing.expect(t, slice.equal(add_result.shape, []uint{4, 2}), "Add result shape should match transposed")
	}
}
