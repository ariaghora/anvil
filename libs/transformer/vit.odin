...
new_conv_2d_bn :: proc(
	...
	conv := nn.new_conv2d(T, in_channels, out_channels, [2]uint{kernel_size, kernel_size}, stride, padding, 1, groups, true, allocator)
	bn := nn.new_batch_norm_2d(T, out_channels, allocator)
	
	return new_clone(Conv_2d_BN(T){
		conv = conv,
		bn = bn,
	}, allocator)
}
...
new_patch_embed :: proc(
	...
	return new_clone(Patch_Embed(T){
		conv1 = conv1,
		conv2 = conv2,
	}, allocator)
}
...
new_patch_merging :: proc(
	...
	return new_clone(Patch_Merging(T){
		conv1 = conv1,
		conv2 = conv2,
		conv3 = conv3,
...
forward_conv_layer :: proc(
	...
	if downsample, has_downsample := layer.downsample.?; has_downsample {
		...
		return tensor.clone(xs, allocator)
	}
}
...
new_attention :: proc(
	...
	// Create attention biases for each head
	seq_len := resolution[0] * resolution[1]
	ab := tensor.zeros(T, []uint{num_heads, seq_len, seq_len}, allocator)
	
	scale := T(1.0 / math.sqrt(f64(key_dim)))
	
	return new_clone(Attention(T){
		norm = norm,
		qkv = qkv,
		proj = proj,
		ab = ab,
...
forward_attention :: proc(
	...
	qkv_reshaped := tensor.reshape(qkv, []uint{b, n, attn.num_heads, attn.key_dim * 2 + attn.d}, context.temp_allocator)
	...
	// Add attention bias
	biased_scores := tensor.add(scaled_scores, attn.ab, context.temp_allocator)
	...
	output_reshaped := tensor.reshape(output_transposed, []uint{b, n, attn.dh}, context.temp_allocator)
...
new_tiny_vit_block :: proc(
	...
	attn := new_attention(T, dim, head_dim, num_heads, 1, [2]uint{window_size, window_size}, allocator)
	mlp := new_mlp(T, dim, dim * MLP_RATIO, allocator)
	local_conv := new_conv_2d_bn(T, dim, dim, LOCAL_CONV_SIZE, 1, LOCAL_CONV_SIZE / 2, dim, allocator)
	
	return new_clone(Tiny_ViT_Block(T){
		attn = attn,
...
forward_tiny_vit_block :: proc(
	...
	xs_4d := tensor.reshape(xs, []uint{b, h, w, c}, context.temp_allocator)
...
new_basic_layer :: proc(
	...
	layer := new(Basic_Layer(T), allocator)
	layer.blocks = blocks
	layer.downsample = downsample_layer
	return layer
}
...
new_tiny_vit_5m :: proc(
	...
	patches_resolution := uint(IMG_SIZE / 4) // 256
	...
	for i_layer in 1 ..< num_layers {
		patches_resolution := patches_resolution / (1 << uint(min(i_layer, 2)))
		layer := new_basic_layer(
			T,
			embed_dims[i_layer], embed_dims[min(i_layer + 1, num_layers - 1)],
			[2]uint{patches_resolution, patches_resolution},
			depths[i_layer],
			num_heads[i_layer],
	...
	neck_conv1 := nn.new_conv2d(T, last_embed_dim, 256, [2]uint{1, 1}, 1, 0, 1, 1, false, allocator)
	neck_ln1 := nn.new_layer_norm_2d(T, []uint{256}, allocator)
	neck_conv2 := nn.new_conv2d(T, 256, 256, [2]uint{3, 3}, 1, 1, 1, 1, false, allocator)
	neck_ln2 := nn.new_layer_norm_2d(T, []uint{256}, allocator)
	
	return new_clone(Tiny_ViT_5m(T){
...
forward_tiny_vit_5m :: proc(
	...
	// Reshape (B, 64*64, C) -> (B, 64, 64, C) -> (B, C, 64, 64)
	xs_4d := tensor.reshape(xs, []uint{b, 64, 64, c}, context.temp_allocator)
	xs_conv := tensor.permute(xs_4d, []uint{0, 3, 1, 2}, context.temp_allocator)
